...
void verify_ISeq( _DOUBLE_ *c, unsigned int m, unsigned int n);
...
void cmdLine(int argc, char *argv[], int& n, int& reps, int& ntx, int& nty, _DOUBLE_ & eps, int& do_host, int& prefer_l1, int& use_rand, int& use_bt, int& use_seq, int& use_shm_double, int& verify_gpu);
...
//      genMatrix_ISeq(h_B, h_A, n);    // Seq * I
